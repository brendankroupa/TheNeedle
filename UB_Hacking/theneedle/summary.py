...
def show_summary(bias_percentage, result, bias_result):
    root = Tk()
    root.title("Bias Summary")

    # Display the bias percentage
    Label(root, text=f"Bias Percentage: {bias_percentage}%").pack()

    # Display the result (e.g., bias leaning)
    Label(root, text=f"Result: {result}").pack()

    # Display the bias result (e.g., detailed statistics)
    Label(root, text=f"Bias Result: {bias_result}").pack()
